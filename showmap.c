...
void showMapodd(int n2 , struct cell * head , struct mapEl** map){
    SetColor(3);
    char c;n2=3;///
    int n = 3 * n2 +2;
    int sum = score(head);
            ...
            if (i%3 == 2){
                     for(j=0;j<=m;j++){///
                        ...
                        if( X == 'B'){
                            ...
                            k2=3505; k1=3505; c=3505; k3=3505;
                        }
                        else if( X == 'A'){
                            X='0';Y=' ';Z=' ';
                        }
...
void updateMap(struct cell * head1 , struct cell * head2 , struct mapEl **map , int n){
    int i=0 , j=0 ,k1=0 ,k2=0;
    struct cell * current1 = head1;
    struct cell * current2 = head2;
    for(  ; current1!= NULL ; current1 = current1->next ){
        map[current1->y][current1->x].room = 1;
    }
    for(  ; current2!= NULL ; current2 = current2->next ){
        map[current2->y][current2->x].room = 1;
    }
}
/*
    for(i=0 ; i<n ; i++){
